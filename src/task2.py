...
def data_preprocessing(line):
        ...
        formatted_date = f"{raw_date[0]}/{raw_date[1]}/{raw_date[2][-2:]}"
        customer_id = parts[1]
        product_id = str(int(parts[5]))

        new_key = f"{formatted_date}-{customer_id}" 
        return (new_key, product_id)
...
if __name__ == "__main__":
    ...
    with open(tempCSV, 'w') as f:
        f.write("DATE-CSUTOMER_ID, PRODUCT_ID\n")
    ...
    # # Preprocessing data 
    ta_feng_rdd = sc.textFile(tempCSV)
    new_header = ta_feng_rdd.first()
    baskets = ta_feng_rdd.filter(lambda line: line != new_header) \
